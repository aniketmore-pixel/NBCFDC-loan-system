...
/** ChartStyle: injects CSS variables per theme for chart colors */
  ...
  const css = Object.entries(THEMES)
    .map(([theme, prefix]) => {
      const body = colorConfig
        .map(([key, itemConfig]) => {
          const color = itemConfig.theme && itemConfig.theme[theme] ? itemConfig.theme[theme] : itemConfig.color;
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const value =
      !labelKey && typeof label === "string"
        ? (config && config[label] && config[label].label) || label
        : itemConfig?.label;
...
function getPayloadConfigFromPayload(config, payload, key) {
  ...
  return config && config[configLabelKey] ? config[configLabelKey] : config && config[key] ? config[key] : undefined;
}
